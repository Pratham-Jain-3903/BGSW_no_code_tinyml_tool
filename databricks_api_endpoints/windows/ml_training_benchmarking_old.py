        ...
        for target_column in available_targets:
            ...
            try:
                ...
                if ml_config["use_case"] == "time_series":
                    ...
                    viz_dir = os.path.join(ml_config["output_dir"], "visualizations", target_column)
                    ...
                    if ml_config["use_pycaret"] and TS_AVAILABLE:
                        try:
                            ...
                            ts_models, ts_metrics = train_and_evaluate_time_series_models(ml_config["time_series"])
                    ...
                    if ml_config["time_series"]["nn_models"] and KERAS_AVAILABLE:
                        try:
                            ...
                            # Combine results
                            ts_models.extend(nn_models)
                            ts_metrics.extend(nn_metrics)
                        except Exception as e:
                    ...
                    # No models trained
                    if len(ts_models) == 0:
                        logger.warning(f"No time series models were trained for {target_column}. Falling back to regression.")
                        
                        # Fallback to standard regression
                        target_df = prepare_ml_data(df.copy(), target_column)
                        setup_pycaret(target_df, target_column, use_case="regression", config=ml_config)
                    ...
                    # Note: We're assuming lower is better for the metric
                    if ts_metrics:
                        # Try to sort by rmse (for neural networks)
                        try:
                            sorted_idx = sorted(range(len(ts_metrics)), 
                                              key=lambda i: ts_metrics[i].get('metrics', {}).get('rmse', float('inf'))
                                                 if isinstance(ts_metrics[i].get('metrics', {}), dict)
                                                 else float('inf'))
                            
                            # Select top models
                            top_n = min(ml_config["top_n_models"], len(ts_metrics))
                            top_models = [ts_models[i] for i in sorted_idx[:top_n]]
                            top_metrics = [ts_metrics[i] for i in sorted_idx[:top_n]]
                        except:
                            # If sorting fails, just use the first N models
                            top_n = min(ml_config["top_n_models"], len(ts_metrics))
                            top_models = ts_models[:top_n]
                            top_metrics = ts_metrics[:top_n]
                        ...
                        all_results[target_column] = {
                            "status": "success",
                            "model_dir": model_dir,
                            "top_model": top_metrics[0]["model_name"] if top_metrics else None,
                        }
