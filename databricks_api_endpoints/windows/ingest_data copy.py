...
# Explicitly set HADOOP_HOME in the script environment as well (optional but can help)
...
default_config = {
    "raw_data_path": "raw_data/all_data/collected_csvs",  # Fix backslashes
    "output_table": "results/ingestion_table",  # Fix backslashes
    "window_size": 2,
...
# 2. Modify process_files_with_sliding_window to process one file at a time
def process_files_with_sliding_window(files: List[str], window_size: int) -> Any:
    ...
    for i, file in enumerate(files):
        try:
            ...
            # Can avoid memory issues with deeply nested transformations
            if (i+1) % 5 == 0:  # Every 5 files
                print(f"Checkpointing after {i+1} files...")
                # Enable checkpointing directory
...
# 3. Modify analyze_data_quality to optimize memory use further
def analyze_data_quality(df: Any, files: List[str], all_columns: set) -> Dict[str, Any]:
    ...
    try:
        ...
        # Use the batch_size from config
        batch_size = config["batch_size"]  # Use the config parameter
...
def run_etl_pipeline() -> Dict[str, Any]:
    ...
    try:
        ...
        full_path_uri = f"file:///{os.path.abspath(delta_output_location).replace('\\', '/')}"
        print(f"Attempting Delta write to: {full_path_uri}")
        try:
            df.write.format("delta").mode("overwrite").save(full_path_uri)
...
def analyze_data():
    ...
    try:
        ...
        data_quality = analyze_data_quality(df, files, all_columns)
        return jsonify({
            "status": "success",
